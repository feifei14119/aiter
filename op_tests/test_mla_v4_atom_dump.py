...
# ATOM_USE_TRITON_ATTN=1 python aiter/op_tests/test_mla_atom_dump.py --from-dump /data/mla_dump --pass triton
...
# --------------------------------------------------------------------------- #
# ATOM's real triton kernel (the one that PRODUCED the dump). Imported DIRECTLY
# from the installed `atom` package — this is the atom-DEPENDENT variant. (The
# standalone counterpart test_mla_v4_triton.py vendors the same kernel instead.)
# --------------------------------------------------------------------------- #
def _atom_triton_decode():
    os.environ.setdefault("ATOM_USE_TRITON_ATTN", "1")
...
def run_from_dump(dump_dir, cos_threshold, which_pass="aiter"):
    ...
    for f in files:
        ...
        # --- triton pass: re-run ATOM's real triton kernel (imported from the
        # atom package) on the dumped inputs; expect it to reproduce dumped o.
        if run_triton:
        ...
        if run_aiter:
            ...
            row["cos(aiter,ref)"] = cos_aiter_ref
            row["cos(atom,ref)"] = cos_atom_ref
        ...
        aiter.logger.info(
            ...
            p["H"],
            p.get("max_kv_len"),
            " ".join(f"{k}={row[k]:.3e}" for k in row if k.startswith("cos(")),
            "passed" if passed else "FAILED",
        )
        if not passed:
            failures.append(
                (os.path.basename(f), {k: row[k] for k in row if k.startswith("cos(")})
            )
...
def _build_parser():
    ...
    parser.add_argument(
        ...
        "  triton - ATOM's real triton kernel (import atom) vs dumped output\n"
