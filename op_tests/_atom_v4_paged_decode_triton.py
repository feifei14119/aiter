...
# VENDORED from ATOM atom/model_ops/v4_kernels/paged_decode.py so the triton
# pass of test_mla_atom_dump.py runs standalone (no `import atom`). Only the
# triton path is kept; ATOM's pure-torch reference (which needed
# `atom.model_ops.sparse_attn_v4._sparse_attn_ragged_torch`) was dropped.
# Sole external deps: torch, triton, aiter (get_num_sms). Keep in sync with
# upstream if the kernel changes.
...
def sparse_attn_v4_paged_decode(
    ...
    Vendored triton-only entry point. ATOM's pure-torch reference and the
    ``ATOM_USE_TRITON_ATTN`` dispatch were intentionally dropped — this always
    runs the triton kernel, which is exactly what the dump was produced with.
