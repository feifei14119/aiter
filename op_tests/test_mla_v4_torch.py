...
"""ATOM-dump driven V4 sparse MLA decode unit test: pure-torch vs golden.

Standalone sibling of test_mla_v4_triton.py, but the non-aiter pass replaces the
triton kernel with a PURE PYTORCH reference implementation of V4 sparse paged
decode (``_reference`` / ``_sparse_attn_torch`` below). No ``import atom`` and no
triton kernel — only torch + aiter. The torch pass re-computes each dumped decode
call in fp32 and compares against ATOM's dumped triton output ``o`` (golden);
because it is a different (naive softmax) implementation, agreement is within
fp32-accumulation / bf16 tolerance rather than bit-exact.
...
# python aiter/op_tests/test_mla_v4_torch.py --from-dump /data/mla_dump --pass torch
...
# --------------------------------------------------------------------------- #
def run_from_dump(dump_dir, cos_threshold, which_pass="torch"):
    ...
    run_torch = which_pass in ("torch", "both")
    ...
    for f in files:
        ...
        # KV in q.dtype for the torch reference (dequant fp8 pool up front).
        ukv_ref = (
        ...
        # --- torch pass: pure-pytorch sparse decode on the dumped inputs, ---- #
        # compared against ATOM's dumped triton output (golden). Different impl
        # (naive fp32 softmax) => agreement within bf16 tolerance, not bit-exact.
        if run_torch:
            out_torch = _reference(
                q, ukv_ref, kv_indices, kv_indptr, attn_sink, softmax_scale
            )
            cos_torch_atom = _cosine_diff(out_torch, o_atom.view_as(out_torch))
            row["cos(torch,atom)"] = cos_torch_atom
            passed = passed and cos_torch_atom < cos_threshold
        ...
        if run_aiter:
            ...
            cos_aiter_torch = _cosine_diff(
                out_aiter,
                _reference(q, ukv_ref, kv_indices, kv_indptr, attn_sink, softmax_scale),
            )
            row["cos(aiter,atom)"] = cos_aiter_atom
            row["cos(aiter,torch)"] = cos_aiter_torch
...
def _build_parser():
    ...
    parser.add_argument(
        "--pass",
        dest="which_pass",
        choices=["aiter", "torch", "both"],
        default="torch",
        help="(dump mode) which kernel to run on the dumped inputs:\n"
        "  torch  - pure-pytorch reference vs dumped output (default)\n"
        "  aiter  - aiter pa_decode_sparse vs dumped output\n"
